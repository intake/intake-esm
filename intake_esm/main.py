...
def _search(query_model: QueryModel, df: pd.DataFrame, columns_with_iterables: set) -> pd.DataFrame:
...
def _search_apply_require_all_on(results: pd.DataFrame, query_model: QueryModel) -> pd.DataFrame:
    ...
    def search(
        ...
        **query:
        ...
        query_model = QueryModel(
            query=query, columns=self.df.columns.tolist(), require_all_on=require_all_on
        )
        query = query_model.normalize_query()
        if not query:
            ...
            results = _search(query_model, self.df, self._columns_with_iterables)
        if query_model.require_all_on is not None and results:
            results = _search_apply_require_all_on(results, query_model)

        new_cat = self.copy()
        new_cat._df = results.reset_index(drop=True)
        return new_cat
